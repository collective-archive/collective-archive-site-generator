...
module.exports = function(grunt) {
  grunt.registerMultiTask('prepare_page_data', 'Convert the extracted CA data into something that Assemble.io can use to generate pages.', function() {
    ...
    var searchFile        = this.data.search;
    var searchTemplateSrc = grunt.file.read('./src/templates/_search_result.hbs');
    var searchTemplate    = Handlebars.compile(searchTemplateSrc);
    ...
    function addToSearchIndex(page) {
      var data = page.data;
      ...
      searchDocs.push(doc);
      searchIndex.add(search);
    }

    function finalSearchObject() {
      return {
        docs:  searchDocs,
    ...
    this.files.forEach(function(file) {
      ...
      pages.forEach(addToSearchIndex);
    ...
    grunt.file.write(searchFile, JSON.stringify(finalSearchObject()));
    grunt.log.writeln("Wrote: " + searchFile);
  });
};
